...
##################################
# Compare mean convergence plots #
##################################

means.df <- data.frame(iteration = 1:its,
                       mean.diff = c(jw.results.old$mean.diffs.vec, jw.results.new$mean.diffs.vec),
                       algorithm = rep(c("Without line search", "With line search"), each = its))

ggplot(data = means.df, aes(x = iteration, y = mean.diff, color = algorithm)) + geom_line()

ggplot(data = filter(means.df, iteration > 200), aes(x = iteration, y = mean.diff, color = algorithm)) + geom_line() +
  geom_abline(slope = 0, intercept = 0, color = "grey50")

table(abs(jw.results.new$mean.diffs.vec) < abs(jw.results.old$mean.diffs.vec))


underLimit <- function(diffs, thresh, start = length(diffs)) {
  if(abs(diffs[start]) < thresh) {
    return(underLimit(diffs, thresh, start = start - 1))
  } else {
    return(start + 1)
  }
}
...
t <- .0001
underLimit(jw.results.old$mean.diffs.vec, t)
underLimit(jw.results.new$mean.diffs.vec, t)
...
max(abs(mle.RA[,its-100] - mle.RA[,its]))
